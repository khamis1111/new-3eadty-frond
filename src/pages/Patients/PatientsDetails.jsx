import { Activity, AlertCircle, Check, Heart, PenSquare, Stethoscope, Trash2 } from 'lucide-react';
...
const PatientsDetails = ({ allAppointment }) => {
    ...
    const GetUser = () => {
        ...
        GetDataToken(`/api/v1/userInfo/${id}`).then((res) => {
            setUser(res.data)
    ...
    const handleUpdateIsPaidOrNot = (value) => {
        if (value === true) {
            PostData(`/api/v1/userInfo/paid/${id}`).then(() => {
            ...
            PostData(`/api/v1/userInfo/notPaid/${id}`).then(() => {
            ...
            <Row>
                ...
                <div className="d-flex gap-2 flex-wrap align-items-center justify-content-between mb-2">
                    ...
                    <Drawer open={isOpen} onOpenChange={setIsOpen}>
                        <DrawerTrigger className="transition-colors focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50">
                            <span className="btn bg-color text-light fw-bold py-2 px-4 shadow"
                ...
                <Col md={8} className='patient-details d-flex flex-column gap-3 p-2'>
                            ...
                            <Card className='h-[290px] p-6'>
                                <div className="d-flex">
                                    <Col>
                                        ...
                                        <p className='m-0'>{user.data.gender === 'male' ? 'ذكر' : 'انثي' || 'لا يوجد'}</p>
                                    </Col>
                                    <Col>
                                ...
                                <div className="d-flex">
                                    <Col>
                                        ...
                                        <p className='m-0'>{user.data.phone || 'لا يوجد'}</p>
                                    </Col>
                                    <Col>
                                    ...
                                    <Col>
                                        {
                                            user.data.paidAt ?
                                                (<>
                                                    ...
                                                    <p className='m-0'>{DateFormate(user.data.paidAt) || 'لا يوجد'}</p>
                                                </>)
                                                :
                                                (<>
                                                    <p className='m-0 text-muted fw-bold fs-6'>المتبقي</p>
                                                    <p className='m-0'>{user.data.restOfPrice || 'لا يوجد'}</p>
                                                </>)
                                        }
                    ...
                    <Card className="w-full">
                        <Tabs defaultValue={localStorage.getItem('TabName') || 'appointments'} className="p-4" style={{ direction: 'rtl' }}
                        ...
                        >
                            <TabsList className="tabs-list w-full grid">
                                ...
                                <TabsTrigger value="allTreatmentsHistory">تاريخ المرض</TabsTrigger>
                                <TabsTrigger value="allTreatmentsPlan">خطة علاج</TabsTrigger>
                                <TabsTrigger value="allTreatmentsDetails">تفاصيل العلاج</TabsTrigger>
                            </TabsList>
                            <TabsContent value="appointments" className='text-end'>
                                <AppointmentsTab user={user.data} allAppointment={allAppointment} />
                            </TabsContent>
                            <TabsContent value="allCurrentMedications">
                                <CurrentMedicationsTab user={user.data.currentMedications} GetUser={GetUser} />
                            </TabsContent>
                            ...
                            <TabsContent value="allTreatmentsHistory">
                                <TreatmentsHistoryTab user={user.data.treatmentsHistory} GetUser={GetUser} />
                            </TabsContent>
                            <TabsContent value="allTreatmentsPlan">
                                <TreatmentsPlanTab user={user.data.treatmentsPlan} GetUser={GetUser} />
                            </TabsContent>
                            <TabsContent value="allTreatmentsDetails">
                                <TreatmentsDetailsTab user={user.data.treatmentsDetails} GetUser={GetUser} />
                            </TabsContent>
                        </Tabs>
                    </Card>
                </Col>
                ...
                <Col md={4} className='d-flex flex-column gap-3 p-2'>
                    ...
                    <Card className="w-full mx-auto">
                            ...
                            {allNotes.map((note) => (
                                <div key={note._id} className="flex items-center justify-between gap-2">
                                    ...
                                    {editingNoteId === note._id ? (
                                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={(e) => handleEditNotes(e, note._id)}>
                                            <Check className="h-4 w-4 text-green-500" />
                                        </Button>
                                    ) : (
                                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditingNoteId(note._id)}>
                                            <PenSquare className="h-4 w-4 text-green-700" />
                                        </Button>
                                    )}
                                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={(e) => handleDeleteNotes(e, note._id)}>
                        ...
                        <CardFooter>
                            <Button value={noteName} onClick={(e) => handleAddNotes(e)}>اضافة ملاحظة</Button>
                        </CardFooter>
                    </Card>
                    {/* MedicalCard */}
                    <Card className="w-full mx-auto">
                        <CardHeader>
                            <CardTitle className="text-2xl font-bold">الملف الطبي الشخصي</CardTitle>
                            <CardDescription>ملخص الحالة الصحية والمعلومات الطبية</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="flex items-center space-x-4 rtl:space-x-reverse">
                                ...
                                <div>
                                    ...
                                    <p className='text-muted'>{user.data.medicalConditions || 'لا يوجد'}</p>
                                </div>
                            </div>
                            <div className="flex items-center space-x-4 rtl:space-x-reverse">
                            ...
                            <div className="flex items-center space-x-4 rtl:space-x-reverse">
                                ...
                                <div>
                                    ...
                                    <p className='text-muted'>{user.data.currentComplaints || 'لا يوجد'}</p>
                                </div>
                            </div>
                            <div className="flex items-center space-x-4 rtl:space-x-reverse">
                                ...
                                <div>
                                    ...
                                    <p className='text-muted'>{user.data.recommendations || 'لا يوجد'}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
